...
export function HomeClient({
  ...
  initialConfirmedParticipantsStatus,
}: {
  initialConfirmedParticipants: ConfirmedParticipant[];
  initialConfirmedParticipantsStatus: "ok" | "empty" | "error";
}) {
  ...
  const confirmedParticipantsStatus = initialConfirmedParticipantsStatus;
  ...
  useEffect(() => {
    ...
    if (isDesktop) {
      ...
      const interval = window.setInterval(() => {
        ...
        if (window.performance.now() < confirmedAutoPauseUntilRef.current)
          return;

        const maxLeft = el.scrollWidth - el.clientWidth;
        if (maxLeft <= 0) return;

        el.scrollLeft += dir * pxPerTick;
        if (el.scrollLeft >= maxLeft - 1) {
  ...
  const scrollConfirmedCarousel = (dir: -1 | 1) => {
    const el = confirmedCarouselRef.current;
    if (!el) return;
    confirmedAutoPauseUntilRef.current = window.performance.now() + 2500;
      ...
      <main className="flex-1">
        ...
        <section className="bg-black/20 backdrop-blur-[2px]">
          <div className="mx-auto w-full max-w-6xl px-4 pb-14 pt-8 sm:px-6 sm:py-16">
            ...
            <div className="mt-5 hidden items-center justify-end gap-2 md:flex relative z-20">
              <button
                type="button"
                aria-label="Participante anterior"
                className="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border border-white/15 bg-black/40 text-white/85 backdrop-blur transition hover:bg-white/10"
                onClick={() => scrollConfirmedCarousel(-1)}
              >
                <span aria-hidden="true">‹</span>
              </button>
              <button
            ...
            <div className="mt-8">
              {confirmedParticipantsStatus === "ok" ? (
                    ...
                    >
                      <div className="flex flex-col items-center gap-3 text-center sm:flex-row sm:items-center sm:gap-4 sm:text-left">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="relative grid size-[72px] shrink-0 cursor-pointer place-items-center select-none"
                      ...
                      {p.descricao ? (
                        <p className="mt-4 whitespace-pre-wrap text-sm italic leading-6 text-white/80 select-none">
                          “{p.descricao}”
                        </p>
                      ) : null}
                    </div>
                ...
                <div className="rounded-3xl border border-white/10 bg-black/55 p-6 text-sm text-white/75">
                  {confirmedParticipantsStatus === "empty"
                    ? "Ainda não temos participantes confirmados."
                    : "Não foi possível carregar os participantes confirmados agora."}
                </div>
              )}
            </div>
          </div>
